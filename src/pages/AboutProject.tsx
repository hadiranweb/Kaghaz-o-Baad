...
import { ScrollChapterProgress } from '@/components/creative';
...
export default function AboutProject() {
  ...
  return (
    <main dir={dir} className="min-h-screen pb-16 bg-background">
      ...
      <div className="container mx-auto grid max-w-6xl gap-8 px-4 pt-14 lg:grid-cols-[10rem_1fr] lg:px-8">
        <aside className="hidden lg:block lg:sticky lg:top-24 lg:self-start" aria-label={text('راهنمای بخش‌های صفحه', 'Page sections')}>
          <ScrollChapterProgress chapters={[
            { id: 'problem', label: text('مسئله', 'Problem') },
            { id: 'process', label: text('فرآیند', 'Process') },
            { id: 'benefits', label: text('مزیت‌ها', 'Benefits') },
            { id: 'audience', label: text('مخاطب', 'Audience') },
            { id: 'faq', label: 'FAQ' },
          ]} />
        </aside>
        <div className="min-w-0 space-y-16">
        <section id="problem" className="scroll-mt-24 grid gap-8 md:grid-cols-2">
      ...
      <footer className="container mx-auto max-w-6xl px-4 pb-4 text-center text-xs text-muted-foreground lg:px-8">{text('آخرین ویرایش: مرداد ۱۴۰۵ — شرح محصول و نقشهٔ پروژه', 'Last updated: August 2026 — Product brief and project blueprint')}</footer>
    </main>
  );
}
